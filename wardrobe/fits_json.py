...
def fit_id(entry: dict) -> str:
    """Stated outright, or from the render filename, or from the hand-written map.

    The file is generated by scripts/export_fits.py now and writes `id` for
    every fit, which is the end of inferring one. The two fallbacks stay for
    the 2026-08-27 file and anything hand-written against its shape: ten fits
    built in the app carry an uploaded photo and no render at all, so there is
    no filename to read an id out of and never was a code.
    """
    stated = entry.get("id")
    if stated:
        return stated
